resource "aws_key_pair" "devopsrole_nginx_deployer" {
  ...
  public_key = file("jenkins_rsa.pub")
}
...
resource "aws_instance" "devopsrole_static_web" {
  ...
  vpc_security_group_ids = [aws_security_group.devopsrole_http_sg.id, aws_security_group.devopsrole_https_sg.id, aws_security_group.devopsrole_ssh_sg.id]
